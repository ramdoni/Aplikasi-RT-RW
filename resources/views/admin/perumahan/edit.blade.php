    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="white-box">
                ...
                <br />
                <form class="form-horizontal" enctype="multipart/form-data" action="{{ route('admin.perumahan.update', $data->id) }}" method="POST">
                    <div class="col-md-6">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <input type="text" class="form-control" required name="nama_perumahan" value="{{ $data->nama_perumahan }}"> </div>
                        </div>
                         <div class="form-group">
                                ...
                                <div  class="col-md-12">
                                    <select name="provinsi_id" class="form-control">
                                        ...
                                        <option value="{{ $item->id_prov }}" {{ $data->provinsi_id == $item->id_prov ? 'selected' : '' }}>{{ $item->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-12">
                                    <select name="kabupaten_id" class="form-control">
                                        ...
                                        @foreach($data->getKabupatenByProvinsi as $item)
                                            <option value="{{ $item->id_kab }}" {{ $data->kabupaten_id == $item->id_kab ? "selected" : '' }}>{{ $item->id_jenis == 1 ? 'KABUPATEN '.$item->nama : 'KOTA '.$item->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div style="background: #f5f5f5;padding:15px;margin-top: 22px;">
                            ...
                            <div class="table-responsive" style="max-height: 300px; overflow: auto;">
                                <table class="table table-striped">
                                    ...
                                    <tbody class="tr-blok">
                                        @foreach($data->getBlok as $no => $item)
                                        <tr>
                                            <td>{{ $no+1 }}</td>
                                            <td>{{ $item->blok }}</td>
                                            <td><a href="{{ route('admin.perumahan.delete-blok', $item->id) }}" class="text-danger" onclick="return confirm('Hapus data ini ?')"><i class="fa fa-trash"></i></a></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
